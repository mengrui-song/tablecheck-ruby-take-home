...
RSpec.describe DynamicPricing::InventoryCalculator, type: :service do
  ...
  describe '#calculate_multiplier' do
    ...
    context 'when demand multiplier is 1.0 (no demand activity)' do
      before { product.update!(quantity: 25) } # Very low stock

      it 'returns 1.0 regardless of inventory level' do
        expect(calculator.calculate_multiplier(1.0)).to eq(1.0)
      end
    end

    context 'with very low stock (1-50) and active demand' do
      ...
      it 'returns base multiplier of 1.3 for Footwear' do
        ...
        expect(calculator.calculate_multiplier(1.2)).to eq(1.37)
      end

      context 'for different categories' do
        it 'applies Footwear premium (1.05)' do
          product.update!(category: 'Footwear')
          expect(calculator.calculate_multiplier(1.2)).to eq(1.37) # 1.3 * 1.05 = 1.365 -> 1.37
        end

        it 'applies Accessories discount (0.95)' do
          product.update!(category: 'Accessories')
          expect(calculator.calculate_multiplier(1.2)).to eq(1.24) # 1.3 * 0.95 = 1.235 -> 1.24
        end

        it 'applies Clothing baseline (1.0)' do
          product.update!(category: 'Clothing')
          expect(calculator.calculate_multiplier(1.2)).to eq(1.3) # 1.3 * 1.0
        end
      end
    end

    context 'with low stock (51-100) and active demand' do
      before { product.update!(quantity: 75) }

      it 'returns base multiplier of 1.2' do
        expect(calculator.calculate_multiplier(1.2)).to eq(1.26) # 1.2 * 1.05 = 1.26
      end
    end

    context 'with medium-low stock (101-175) and active demand' do
      before { product.update!(quantity: 150) }

      it 'returns base multiplier of 1.1' do
        expect(calculator.calculate_multiplier(1.2)).to eq(1.16) # 1.1 * 1.05 = 1.155 -> 1.16
      end
    end

    context 'with good stock (176-250) and active demand' do
      before { product.update!(quantity: 200) }

      it 'returns base multiplier of 1.0' do
        expect(calculator.calculate_multiplier(1.2)).to eq(1.05) # 1.0 * 1.05 = 1.05
      end
    end

    context 'with high stock (251+) and active demand' do
      before { product.update!(quantity: 280) }

      it 'returns base multiplier of 0.9' do
        expect(calculator.calculate_multiplier(1.2)).to eq(0.95) # 0.9 * 1.05 = 0.945 -> 0.95
      end
    end
  end
  ...
  describe 'integration test with actual inventory data' do
    context 'with real inventory examples and active demand' do
      it 'handles A-Team Headband (lowest quantity: 23, Accessories)' do
        ...
        expect(calculator.calculate_multiplier(1.2)).to eq(1.24)
      end
      ...
      it 'handles Michael Jackson Glove (highest quantity: 292, Accessories)' do
        ...
        expect(calculator.calculate_multiplier(1.2)).to eq(0.86)
      end
      ...
      it 'handles Ghostbusters T-Shirt (medium quantity: 141, Clothing)' do
        ...
        expect(calculator.calculate_multiplier(1.2)).to eq(1.1)
      end
      ...
      it 'handles MC Hammer Pants (high quantity: 285, Footwear)' do
        ...
        expect(calculator.calculate_multiplier(1.2)).to eq(0.95)
      end
    end

    context 'with no demand activity (demand multiplier = 1.0)' do
      it 'returns 1.0 regardless of inventory level' do
        product.update!(quantity: 23, category: 'Accessories')
        expect(calculator.calculate_multiplier(1.0)).to eq(1.0)
  ...
  describe 'boundary testing' do
    context 'at quantity thresholds with active demand' do
      it 'handles boundary between very low and low stock (50->51)' do
        product.update!(quantity: 50, category: 'Clothing')
        expect(calculator.calculate_multiplier(1.2)).to eq(1.3)

        product.update!(quantity: 51, category: 'Clothing')
        expect(calculator.calculate_multiplier(1.2)).to eq(1.2)
      end
      ...
      it 'handles boundary between medium-low and good stock (175->176)' do
        ...
        expect(calculator.calculate_multiplier(1.2)).to eq(1.0)
      end
      ...
      it 'handles boundary between good and high stock (250->251)' do
        ...
        expect(calculator.calculate_multiplier(1.2)).to eq(0.9)
      end
    end
  end

  describe 'edge cases' do
    context 'with active demand' do
    ...
    context 'with no demand activity' do
      it 'returns 1.0 for any quantity when demand is inactive' do
        product.update!(quantity: 10000, category: 'Clothing')
        expect(calculator.calculate_multiplier(1.0)).to eq(1.0)
