...
RSpec.describe DynamicPricingService, 'Integration Tests', type: :service do
  describe '.update_all_prices' do
    ...
    it 'updates prices for all products using real calculation logic' do
      ...
      # - Inventory: 1.0 (no inventory adjustment when demand is 1.0)
      # - Competitor: high competitor prices (8968) - no reduction needed
      # - Floor: 80% protection applied

      # Low inventory (5) but demand is 1.0, so inventory multiplier is also 1.0
      expect(gadget.dynamic_price).to eq(500)

      # Medium inventory (100) but demand is 1.0, so inventory multiplier is 1.0: 800 * 1.0 * 1.0 = 800
      expect(book.dynamic_price).to eq(800)

      # Very low inventory (30) but demand is 1.0, so inventory multiplier is 1.0: 1200 * 1.0 * 1.0 = 1200
      expect(clothing.dynamic_price).to eq(1200)
